...
Две причины: простота и асинхронность. Простота в том, что сверстать приложение может любой школьник, который умеет на html/css/javascript
...
В этом json есть только одно служебное поле, оно называется block. Остальные поля, которые вы видите, могут называться как вам нравится.
...
А если отойти от картины - Оооо, так это де Шишкин Иван Иванович с его знаменитыми мишками.
...
Какой веб-фреймворк для верстки ни возьми, он в какой-то степени напоминает хлебопечку. Или пульт от навороченного телевизора.
...
видит не теги, которые будет в html-файле. Он видит какие-то абстрактные блоки. Ага - это будет шапка, это подвал,
...
Скорее всего вы называете эти же блоки как-то иначе. Или вообще не называете, потому что надобности в этом
не испытываете :)

А если вы не верстальщик, а настоящий программист, то вам станет яснее, когда я скажу, что блоком мы называем
примерно такую же абстракцию, как паттерн "Модуль" в javascript.
...
другого механизма, кроме ручного перебора каждой страницы. А если у вас не дайбожэ SPA, и часть блоков не видна,
...
Кроме меня. Я относился к CSS-препроцессорам брезгливо. "Баловство это", — я так считал, — "Тот же CSS, только с синтаксическим сахаром."
...
со всеми вью, какие у вас есть. Её легко открыть в браузере и проверить.
...
Другие подходы используют css-классы, чтобы находить блоки в дом-дереве. Тот же jQuery с функцией $, да многие другие.
...
Для каждого блока мы описываем, что он должен делать. Здесь жэ описываем реакции на пользовательские события.
...
Модуль - это очень простая идея.
Модуль — это специальный js-объект, внутрь которого мы вкладываем свой программный код. Ваш код будет работать, как и прежде, в нём ничего не меняется.
А модуль нужен лишь для того, чтобы обеспечить взаимодействие с другими фрагментами вашего кода, другими вашими модулями. Модуль, как кассир в Макдональдсе —
когда он готов работать - он вскидывает руку: "Свободная касса!". Когда модуль загрузился в память, он сообщает об этом в модульную систему. И другие
модули с этого момента могут использовать ваш код из этого модуля.

Прям как по domReady.
...
Третья - массив других модулей. Пока они не скажут про себя, что они готовы, модуль form ничего не начнёт делать.

Четвертая - содержимое модуля. Именно в этой функции и происходит программирование поведения блока.
...
Потому что это настоящее ООП, с классами, с инстансами, с асинхронностью. Потому что здесь есть четкое разделение приватных и публичных методов.
...
```
Слово, записанное в атрибуте class участвует исключительно в стилевом отображении.
А то, что написано в `data-block-name` - именно по этому атрибуту Бивис находит шапку и назначает ей js-поведение.
Оказалось, что использовать дата-атрибуты для построения виртуального дерева блока на странице гораздо быстрее, чем использовать css-классы.
...
Единственное место, где поведение и отображение пересекаются, это состояния блока. Этого никак не избежать, потому что и в жизни оно так устроено.
...
А когда учитель хочет проверить у всего класса собрать тетради на проверку, она ходит по рядам и забирает тетради в стопку.
...
изменения на файловой системе и пересобирает файлы.
...
Весь этот код покрыт юнит-тестами. Он написан в очень понятном кодстайле, которого придерживается
команда Яндекс.Карт.
