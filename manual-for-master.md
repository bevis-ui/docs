...
Это набор `PHP`/`Python`-команд, которые ходят в бекенд за данными.
...
Это абсолютно валидный ``JS`ON`. Совершенно обычный ``JS`ON`. Бивис создавался, как фреймворк, который использует совершенно валидные нативные конструкции.
В этом ``JS`ON` есть только одно служебное поле, оно называется block. Остальные поля, которые вы видите, могут называться как вам нравится.

Почему блок? Зачем какой-то ``JS`ON`? И что оно вообще такое?

Подойдите вплотную к любой картине. Подойдите так близко, чтобы касаться её носом. Что-то видно?
А виден только хаос из цветных мазочков. Ни черта не разобрать.
...
видим не теги, которые будет в `HTML`-файле. Мы видим какие-то абстрактные блоки. Ага - это будет шапка, это подвал, а тут форма авторизации.

Вы тоже, скорее всего, разбиваете страницу на небольшие части, и называете их про себя модулями или компонентами или контролами.
Мы называем эти абстракции `блоками`, потому что мы из Яндекса, а там родилась концепция верстки абсолютно независимыми блоками,
...
Бивис-блок - это очень простая декларация на ``JS`ON`. Простота - это очень важно.
...
Вы задумывались, что пожилым людям из всего пульта нужно три кнопки: включить, сделать громче и переключить канал?
...
И мы в Бивисе сделали одну крутую вещь, которой гордимся-гордимся: чтобы на странице появился блок, его достаточно позвать на странице только по имени.
Всё, больше о нём мне ничего знать не нужно. Вот, вообще ничего.
...
Фактически, мы видим декларацию только одного блока. Не смотрите на всю мишуру вокруг, сконцентрируйтесь только на этом ``JS`ON`:
...
Мы хотим, чтобы из этого ``JS`ON`-а на странице появился такой `HTML`:
```html
...
То есть мы делам блок button с `view: "normal-shadow"`. В ``JS`ON` это записывается так:
...
Модульные системы — это и `Require`JS``, и `Common`JS``.
...
Работая над проектом достаточно лишь обновлять страницу в браузере, чтобы всегда видеть актуальные стили, скрипты и верстку. Наш
[ENB](https://github.com/enb-make/enb)-сборщик отличается огромной скоростью сборки для проектов с блочной структурой.
Он собирает проекты со скоростью пули.
