...
Вместе с тем, для каждого сочетания кнопочных модификаторов требуется корректная работа в паре с
...
=====================
Напомню, у блока может быть представление, мы называем его `view`.
Ещё блок может находиться в разных состояниях. Их мы называем `state`. Состояния похожи на `BEM`-модификаторы,
...
для проекта и получил быстрый профит, а сейчас вдруг без твоего ведома эта кнопка сломалась. А ты этого не хотел.
...
Библиотека не имеет влияние на твой проект, только ты (самостоятельно) решаешь, когда в проект добавить исправление для ошибки в библиотечной
кнопке. У тебя и так бага висит в продакшен-кнопке, твои пользователи с ней знакомы. То есть, при неполучении фикса из библиотеки,
...
причины, для примера не важно, — примем на веру, что они есть. Что делаем?
