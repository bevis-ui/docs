На YaC2013 звучала мысль, что `BEViS` – это диалект `BEM`, но со строгим `API`. Эта мысль правдива только наполовину.
 На ту  половину, которая про АПИ. `BEViS` не имеет отношения к `BEM`. Ну, разве только косвенное,
 потому как и `BEM` и `BEViS` – это две ветви развития концепции абсолютно независимых блоков (далее — `АНБ`).
 Правильно утверждать так:

`
BEViS – это АНБ, но со строгим АПИ
`

Строгость API внутри `BEViS` заключается в том, что в разработке и использовании блоков разрешены только такие
подходы, которые гарантируют обратную совместимость кода. Другими словами, подключите `BEViS`-библиотеку в свой
проект, и у вас ничего не сломается при следующем обновлении библиотеки.
...
Сравните его с описанием в [islands-page](https://github.yandex-team.ru/pages/lego/islands-page/v1.50.0/catalogue/header.html)
...
И это ещё неполный пример шапки из `lego-islands`, он реализует не всю функциональность,
которая есть в сравниваемой `btjson`-шапке. То есть полный лего-пример будет ещё подробнее.
...
В результате разделения "сфер ответственности" код проекта стал проще и понятнее.
