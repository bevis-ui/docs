...
В момент инициализации `Input` мы ожидаем, что выполнится метод-конструктор, а нём вызовется метод
`this.getValue()`, и его результат окажется в браузерной консоли.
...
Потому что по дефолту в Бивисе отключена автоматическая инициализация всех блоков. Это экономно расходует память
браузера, это даёт разработчику полный контроль. Решение о том, нужно инициализировать все блоки сразу во время
загрузки страницы (или только какие-то блоки "по мере их готовности") принимает разработчик проекта.

Если мы хотим, чтобы бивис инициализировал все блоки на странице автоматически, во время загрузки страницы (а мы
именно этого сейчас и хотим), нам нужно укзаать дополнительную зависимость для страницы.

Снова откройте файл `pages/test-page/test-page.deps.yaml` и допишите новую зависимость:
...
Мы только что добавили зависимость от блока по имени `block` (случайная тавтология),
а точнее от его элемента `auto-init`, который и занимается тем, что инициализирует все ваши блоки при загрузке
страницы.

Теперь обновите страницу в браузере. Успех? Я вижу в консоли ровно тот же текст,
который есть сейчас в текстовом поле. Вы тоже видите? Это отработал метод `getValue()`.
...
Оставшиеся 10% - узкие прикладные задачи, которые мы решим в отдельных руководствах:
...
* про все способы инициализации блока
* про доступ и операции над элементами блока
* про навешивание событий

В [документе про BT](bt.md) мы расскажем:
* Как строить внутреннюю структуру блока
* Как легко параметрами менять внутреннее устройство блока
