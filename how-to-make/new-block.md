...
Что мы понимаем, когда смотрим на составленные `UML`-схемы? Наш блок довольно простой: значение из него можно
а) получить и б) установить.

Отлично, теперь мы готовы писать актуальный шаблон, который
а) должен генерить блок в виде тега `input`.
б) должен уметь устанавливать в тег значение, которые мы задали ему при декларации блока.
