...
Здесь мы захаодкодили имя будущего атрибута. Не зазорно :)
Если мы точно значем, что имя у инпута никогда не поменяется, почему бы и "не прибить гвоздями"?

А если серьёзно, то этим шагом мы не столько "хардкодим", сколько изолируем от внешнего мира внутреннее
устройство блока. Мы не разрешаем извне передавать в инпут эту информацию. Мы ограничиваем API этого блока. И если мы
 делаем это сознательно по каким-то важным для нас причинам... почему бы и нет?

То есть мы можем делать API блока более открытым или менее открытым. Это решать конкретно вам. Мы старемся открывать
блок наружу только самым минимумом. Потому что тогда мы сохраняем максимальный контроль над внутренним устройством
блока.

Например, сейчас мы выливаем шаблоном сразу тег `input`, а завтра вдруг окажется, что вокруг инпута нужна обёртка из
одного `div`. Так как в API блока максимально простое (имя + минимум параметров),
мы можем как угодно менять html-реализацию. Правим или пишем новые шаблоны, котрые генерят другой html. Главное,
чтобы эти шаблоны принимали тот минимум параметров. А использовать их шаблоны могут как хочешь свободно.
