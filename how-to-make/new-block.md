...
он только лишь значение отдавать или получать.

Отлично, теперь мы готовы написать актуальный шаблон, который должен уметь...
...
_Обратите внимание, в бивисе нет зарезервированных полей, кроме `block` и `view`._

_Вместо `value: 'Привет,
...
И вот структура нашего блока уже почти готова. Но только какое текстовое поле формы может существовать без атрибута
`name`? Как форма передаст его значение на сервер? Нам нужно выливать в блок этот атрибут.

Вы уже знаете, как. Можно было бы и не показывать:
