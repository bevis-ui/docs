...
То, что мы написали, называется модуль. Модульных систем много разных, бивис использует
[Ymaps Modules](https://github.com/ymaps/modules/blob/master/what-is-this.md), которые придумали и разработали
разработчики из Яндекс.Карт. Пройдите по ссылке, прочитайте. В основу модулей легла простая идея,
и в документации простым языком описывается, что означает каждая строчка из кода,
котрый мы привели выше.
