...
Создадим файл для `view: large`, назовём его `input_large.styl`. В него из файла `input.styl` перенесём селектор для 
этого вью:
...
В файле `input.styl` останется только код миксина `input_mixin($height) {...}`.
...
сразу его переименуйте в `input_skin_base.styl`. 
...
нахлдящихся в папке `_skin` (это следует из инструкции `- skin: "*"`). При этом миксины должны попасть в сборку раньше, 
