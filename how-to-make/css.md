...
И когда мы в шаблоне сказали, к примеру, `var placeholder = ctx.getParam('placeholder')`, в локальной переменной
`placeholder` окажется строка "на сайте", которую мы указали в декларации страницы. То есть мы получили значение
параметра из контекста.
