...
2. После этого Бивис ищет `js`-модуль, соответсвтующий этой ноду и находит его — `modules.define( 'form', //...);`.
 
3. Находит в модуле метод `__constructor` и последовательно выполняет все его инструкции. 

4. И так для каждого бивис-блока, который есть в дереве документа. 
