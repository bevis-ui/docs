...
Самое время запрограммировать поведение блока. Сделать блок интерактивным — отзывчивым на действия пользователя.
...
Мы пишем клиентский `javascript` в виде модулей. Простыми словами - пишем `javascript`-программу и помещаем её в
специальный контейнер, который называют модулем.
...
дополнительные методы описываются во втором аргументе функции `inherit`.

В этом коде мы создаём класс `Input`, как наследника от класса `Yblock`, и насыщаем его собственными методами `getValue`
и `setValue`:
...
Метод `__constructor` - тоже метод нового класса `Input`. Но если другие методы мы вольны называть как нам нравится, то
...
Интерпетатор входит в ваш метод `__constructor` и выполняет всё, что вы там написали.
...
Вообще-то это способ из конструктора класса `Input` вызвать конструктор класса `YBlock`.

То есть, вы описали класс `Input` как наследника от класса `YBlock`. А потом создали экземпляр `myInput`. В тот момент,
когда вы создали объект `myInput`, вызвался его конструктор, который вызвал точно такой же конструктор в `YBlock`.

Если строка `this.__base.apply(this, arguments);` выглядит пугающе, не обращайте внимания, эта строка неинтересная. 
Интересно другое.

С помощью модуля `inherit` мы пишем клиенсткий `javascript`-код в ООП-стиле. Не в виде спагетти-функций, которые друг
