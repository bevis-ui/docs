...
Из названия и `JSDoc` следует, что метод занимается рендерингом всех контролов в форме. Рендерингом мы между 
собой привыкли называть отображение какого-то блока в `DOM`-дереве. То есть, конечно, метод, ничего не рендерит в 
прямом смысле этого слова, он лишь вставляет какие-то блоки в `DOM`-дерево. А рендерит их (отображает, навешивает 
стили и т.д) уже сам браузер. Слава богу, нам этого делать не надо!
