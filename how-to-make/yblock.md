...
Мы выносим тело обработчика в отдельный метод класса, а методе `on` указываем только имя обработчика. 
...
То есть когда метод `emit` выкинул вверх событие `input-submitted`, он заодно передал слушателю некие данные. Помните 
второй параметр? Вот так мы эмитили событие и передавали некий объект с полем `value`:
...
И когда метод `on` услышал это событие, он вызвал функцию `_onInputSubmitted`. Но не просто вызвал, а ещё и передал в
 неё некий специальный объект - `Event`. Это вообще-то обычное дело, что я тут объясняю?! Коллбек-обработчик всегда 
 получает единственный параметр `Event`. Так происходит и в `jQuery` и в `BEViS`. Поэтому обработчик ждёт в 
 агрументах вызова этот объект. Традиционно его принято принимать в переменную `e`. Мы и принимаем:
...
передавать его в `_onInputSubmitted`, но метод `_onInputSubmitted` принимать евент не будет. А вообще стоит принимать
 его :)
  
Если мы сейчас посмотрим в браузер (обновим страницу, конечно), то можно увидеть, что когда мы установили фокус в 
инпут и нажали `Enter`, в консоли появилось сообщение 
```javascript
...
```
Мы смотрим на объект `YEventEmitter` — тот самый евент, который приходит в каждый обработчик события. В нём мы видтм 
поле `data`, в которм оказались те данные, которые отдал `Input`, когда эмитил событие. Мы видим тип события в поле 
