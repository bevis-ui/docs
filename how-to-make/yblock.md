...
мы создали всегда только один экземпляр класса `Input`:
```javascript
// Создаём инпут
this._greetingInput = new Input({
    value: 'Привет, Бивис',
...
динамическими методами и 2 статическими. Значит памяти будет занято столько, чтобы уместилось 12 динамических методов
 и 2 статических.
 
А что будет с памятью компютера, когда мы в файле `form.js` создадим два экземпляра Класса `Input`? Например, 
...
Будет три экземпляра инпута — память будет раходоваться на 36 динамических методов и ... 2 статических.
...
знать — этот метод должен возвращать точно такую же строку, что записана в имени модуля (см. первый аргумент 
`modules.define`)  
