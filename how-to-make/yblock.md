...
Мы в работе пользуемся всеми тремя способами одновременно: что можно сгенерить статически - генерим на сервере; 
если какие-то данные для генерации блока мы можем получить только в браузере, мы генерим блок динамически.

Следует добавить, что создавать бивис-блоки налету можно ещё одним способом.
...
Так же можно создавать элементы блока, если в опциях указать поле `elem`
```javascript
...
Обратите внимание, метод `apply` возвращает именно `HTML` блока или элемента, но в виде строки. Не знаю, зачем вам 
это может пригодиться, я не могу вспомнить сходу случая, когда это мне было нужно.
...
`Javascript`-интерпретатор его прочитает, и как только модульная система загрузит два других модуля,указанных в 
зависимостях: `jquery`, `block`.

Именно этот файл - точка входа в работу нашего javascript-bevis-приложения. И работает это так:

Слушай меня, `Javascript`-интерпретатор. Как только модульная система загрузит модули `jquery` и `block`, сразу выполни
 следующий код:
```javascript
    $(function () {
        Block.initDomTree(window.document).done();
...
```javascript 
        Block.initDomTree(window.document).done();
``` 
А в этой строке вызывается статический метод `initDomTree` у модуля `YBlock` (сейчас в документации и коде 
`bevis-stub` есть легкая путаница с именем `Block`/`YBlock` — это один и тот же модуль, я позже наведу порядок в этом
 деле).

А метод `Block.initDomTree(window.document)` иницилизирует все бивис-компоненты, который есть в этот момент в  
`DOM`-дереве ноды `window .document`. 

"Инициализирует" - означает, что Бивис обходит всё дерево документа и встречает в нём, к примеру, ноду 
`<div class="form">`. После этого Бивис находит модуль, который мы создавали с вами `modules.define( 'form', //...);`, 
находит в нём метод `__constructor` и выполняет все его инструкции. И так для каждого бивис-блока, который есть в 
дереве документа. 


Файл `core/block/block__auto-init.js` попадает в собранный `test.ru.js` благодаря тому, что в зависимостях мы указали: 


У вас наверняка уже возникал вопрос, 
----
