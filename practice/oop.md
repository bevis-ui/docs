...
новый экземпляр класса с помощью оператора `var data = new Array()` (или с помощью шотката `var data = []`, что одно
 и то же ) и сохранили его в объекте `data`, созданный объектсразу приобрел сразу все характерные признаки класса 
 `Array`.
...
Вы знаете, как описывать функции? Конечно, вы пишете не меньше десятка новых функций в день:
Или так:
```javascript
function getValue () {
    // ...  
...
А знаете, как описывать классы? Вот это новость:
```javascript
function Input () {
    // ...
}
...
Забавно? Любая функция может стать классом. Супер.

_Обратите внимание, имя класса начинается c заглавной — `Input`. Это неписаное соглашение среди ООП-программистов: 
"Классы именуем с заглавной". Его можно игнорироать, но лучше, конечно, ему следовать, чтобы вы сами могли отличить 
по именам, где простая функция, а где целый класс._
...
инстанцию класса). То есть я создал конкретный инпут. Да, этот инпут пока ничего не умеет - ни `HTML` для себя 
создавать, ни реагировать на действия пользователя — ничего. Он умеет лишь хранить в себе поле `name` и заполнять 
его данными, которые передали при инстанцировании класса `Input`.
...
чтобы в салон могли сесть пассажиры. С точки зрения ООП эти все умения называются `методами` автомобиля.

_То есть свойства объекта мы в ООП называем "полями", а умения объекта называем "методами". Почему такие названия, 
сейчас не важно. Просто  запомните: когда встретите слова "поле объекта" или "поле класса", вспоминайте свойства 
автомобиля. А когда встретите "метод объекта" или "метод класса", вспоминайте умения автомобиля._
