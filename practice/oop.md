...
тщательно продумывают, создают и документируют (например, с помощью `JSDoc`). А после этого больше никогда их не 
меняют! 
...
другого класса с помощью _публичных_ методов. Если вдруг вы отрефакторите какой-то класс и убьёте публичный метод 
...
педали сцепления, тормоза и газа! Отрефакторил, понимаешь! Куда я уеду? Да, никуда! Просто потому что я, как
объект класса `Человек`, научился работать с машиной как с объектом класса `Автомобиль` при помощи публичного метода
`Автомобиль.ПедальГаза()` и `Автомобиль.ПедальТормоза()` в определенном порядке — слева сцепление, справа газ, а 
между ними тормоз. Этот порядок вшит в мою голову намертво. Я даже если попытаюсь уехать из СТО, приноравливаясь к 
новому расположению педалей, я на первом же (первом!) перекрестке создам ДТП. Потому что **ломать публичный интерфейс
 нельзя**!

Приватные методы — меняйте, удаляйте или добавляйте хоть трижды в день и до завтрака ещё пару раз. Но публичные 
методы а) не должны исчезать и б) должны выполнять ту функцию, ради которой публичный методы были созданы.
