...
момент, когда этот метод уже был однократно вызван из конструктора класса. И ещё странно, что в текуей реализации 
...
объект класса Человек, научился работать с машиной как с объектом класса Автомобиль при помощи публичных методов  
интерфейса типа педалей в определенном порядке — слева сцепление, справа газ, а между ними тормоз. Этот порядок вшит в
 мою голову намертво. Я даже если попытаюсь уехать из СТО, приноравливаясь к новому расположению педалей, я на первом
  же (первом!) перекрестке создам ДТП. Потому что **нельзя ломать публичный интерфейс**!
...
В ООП не так всё сурово, к счастью. Не нарушайте :)
