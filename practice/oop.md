...
Как любая реальная машина имеет 4 колеса, двигатель и салон, так и любой объект, созданный из класса `String` имеет
...
Зачем нужно возвращать `this`? И вообще что такое `this`?

_`This` — ключевое слово языка `Javascript`, которое указывает на текущий контекст выполнения операции._

Брр, скулы сводит от таких казённых объяснений. А можно ли как-то по-человечески?

Давайте так попробуем. Любая функция может возвращать результат своей работы. Делается это с помощью оператора
`return`. В нашем случае функция `Input` возвращает саму себя с помощью `return this;`. Вот так и выходит, что в
переменной `inputLogin` оказывается объект класса `Input`. То есть у переменной `inputLogin`  вдруг откуда ни
возьмись появляется поле `name`, хотя мы его явным образом внутри этой переменной не создавали.
...
- А следующей строкой устанавливаем ему свойство `this.domElement.name`. Оказывается, у объектов класса `HTMLElement` есть своё собственное поле `name`, в которое мы можем передать значение. Этим и занимаемся - принимаем значение из параметра `params` функции-конструктора (нам приходит хеш с одним ключом `nameAttr`).
...
_О первых двух мы уже поговрили выше, это были принципы абстракции и инкапсуляции. О четвертом - полиморфизме 
...
очень любопытно, как наследовать с помощью прототипов, почитайте у Ильи Кантора статью учебника
["Наследование классов в JavaScript"](https://learn.javascript.ru/class-inheritance)._
