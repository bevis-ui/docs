...
## Вообще-то, вы умеете писать объектно-ориентированный код!
...
Когда вы создаёте переменную типа `Array`, на самом деле вы создаёте объект класса `Array`. У слова `объект класса` 
есть ещё синоним — `экземпляр класса`. Объект и экземпляр - два разных слова, описывающие одно и то же. Оба 
слова звучат замысловато, даже чужеродно. Но так принято, все `ООП`-адепты выражаются именно такими словами и друг 
друга понимают. И мы привыкнем, и мы научимся. Давайте это сделаем на примере из жизни.

Мы, простые люди, говорим "Порода собак", а ООП-программисты это же понятие называют "Класс собак". Мы говорим
 "представитель такой-то породы собак", а программист говорит "экземпляр такого-то класса" (или "объект такого-то 
 класса").   
...
Для вас это, наверняка, никакая не новость. Но задумайтесь — мы написали один простой оператор `new Image()` и получили 
...
Я не стал писать код, отправляющий данные на сервер, для нашей темы это сейчас не важно, — будем выливать значение поля
 в консоль браузера, этого достаточно. В общем виде мы реализовали задумку. Скорее всего, код можно оптимизировать, —
...
_Об операторе `new` можно почитать в учебнике Ильи Кантора: 
[https://learn.javascript.ru/constructor-new](https://learn.javascript.ru/constructor-new)._ 

Но читать прямо сейчас необязательно. Вы уже поняли главное — оператор `new` нужен для того, чтобы создать инстанс 
какого-то класса, — другими словами, — экземпляр, конкретный объект. Говоря простыми словами, с помощью этого оператора 
рождается новый объект. А что именно происходит под капотом этого оператора, знать прямо сейчас совершенно не важно. 
Важно понимать одно - для создания инстансов класса в языке `Javascript` всё есть.
...
чтобы в салон могли сесть пассажиры. С точки зрения ООП эти все умения, эти все действия, называются `методами` 
автомобиля.
...
Я подразумевал тот факт, что не существует красивого нативного способа описывать **методы класса**. 
...
_Если вам очень-преочень любопытно, Илья Кантор смог это объяснить [хорошо](https://learn.javascript.ru/prototype).  Но
лучше не надо, не читайте пока, поберегите себя. Прочитайте через пару недель. Или через пару-тройку месяцев._

Вам совершенно нет нужды понимать, что такое прототипы, чтобы уметь ими пользоваться. Я, к примеру, не разбираюсь в 
устройстве своего автомобиля, но езжу на нём каждый день. И с прототипами точно так же.
...
В чём главное преимущество ООП перед процедурным стилем? В том, что все свойства и умения конкретного объекта мы
...
`ООП` — это всего лишь ещё один способ организации  своей `Javascript`-программы.
...
`Объект класса` (он же объект какого-то типа, как мы его раньше называли) - это обычный `Javascript`-объект, 
возникший благодаря выполнению того кода, который мы описали в `Классе`.
  
_`Класс` — это формочка для выпекания, а `Объект класса` — пирог, запеченный в этой формочке. `Класс` — это 
штамп, а `Объект класса` — оттиск... Если эти примеры вам не близки, придумайте свои аналогии. Главное, чтобы вы чётко
 понимали связь между понятиями `Класс` и `Объект класса`._
 
Раньше мы группировали свой код преимущественно в функции, а с переходом на `ООП` предпочитаем организовывать код в 
некие `Классы`, из которых потом создаём `Объекты класса`. Процесс создания `Объекта класса` ещё называют  
`инициализацией`. После инициализации каждый `Объект класса` приобретает какие-то свойства и умения, характерные для
 своего `Класса`. В `ООП` свойства и умениия описываются с помощью полей и методов объекта. Ещё мы узнали, что 
 `Объект`, произведенный от конкретного `Класса`, принято называть `экземпляром` этого класса, или `инстансом`, или 
 `инстанцией`, — кому как нравится.

В `Javascript`-е есть встроенные классы. Каждый из нас, — неопытных `Javascript`-разработчиков, —  бесчисленное 
количество раз создавал объекты от встроенных классов, не подозревая об этом: когда мы создавали строковую 
переменную - мы как бы создавали инстанс класса `String`. Когда мы создавали массив — мы на самом деле 
создавали инстанс класса `Array`.
...
Создать `экземпляр класса` очень просто. Этим занимается оператор `new`.
...
function Input (params) {
    ...
    // Хочется объявлять методы прямо в констукторе.
    this.onFocused = function (e) {
         e.target.style.border = '1px solid yellow';
    };
}
...
прототипы. Хочется объявлять методы прямо в констукторе.
...
ничего не делает. А потом мы - программисты — решаем, как себя вести объекту. Говорим ему — отобразись внутри 
`document.body`. А захотим, отобразим его в любом другом `HTML`-элементе.

Публичный интерфейс ломать нельзя!

При создании классов принято тщательно продумывать публичный интерфейс — какие методы будут "светиться" наружу. Их 
тщательно продумывают, создают и документируют (например, с помощью `JSDoc`). А после никогда не меняют! 

_Представляете, какая ответственность ложится на плечи программиста, который вводит в свой класс публичный метод?_
 
Публичные методы больше не меняют, потому что в `ООП`-программе объект одного класса взаимодействует с объектом 
другого класса посредством _публичных_ методов. Если вдруг вы отрефакторите какой-то класс и убьёте публичный метод 
(или даже поменяете местами ожидаемые параметры в этом методе), скорее всего ваша программа сломается.

Отдал я машину на Тех.Обслуживание. Через два часа вернулся, завёл машину, трогаюсь с места... а механик, оказывается, 
переставил местами педали сцепления, тормоза и газа. Отрефакторил, понимаешь! Куда я уеду? Да, никуда! Просто потому
 что я, как объект класса `Человек`, научился работать с машиной как с объектом класса `Автомобиль` при помощи 
 публичного метода `Автомобиль.ПедальСцепления()` и `Автомобиль.ПедальГаза()` в определенном порядке — слева 
 сцепление, справа газ, а между ними тормоз. Этот порядок вшит в мою голову намертво. Я даже если попытаюсь уехать 
 из СТО, приноравливаясь к новому расположению педалей, я на первом же (первом!) перекрестке создам ДТП. Потому что 
 **ломать публичный интерфейс нельзя**!

Приватные методы — меняйте, удаляйте или добавляйте хоть трижды в день и до завтрака ещё пару раз. Но публичные 
методы а) не должны исчезать и б) должны выполнять ту функцию, ради которой были созданы.

Например, взбрело мне что-то в голову, и я переписал класс `Input` полностью, но _не тронул_  единственный публичный 
метод `render`:
```javascript
var Input = inherit({
    __constructor: function (params) {
...
C точки зрения пользователя этого класса не изменилось ничего. Ведь как мы пользовались этим классом? А вот как:
```javascript
var inputLogin = new Inpit({nameAttr: 'login'});
inputLogin.render(document.body);
```
Мы только создавали инстанс класса, а потом вызывали публичный метод `render`. После рефакторинга эта схема не 
изменилась вообще — всё так и осталось.

А с точки зрения внутреннего устройства класса поменялось всё — я удалил все приватные методы. Вся работа теперь
возложена на конструктор. Но только благодаря тому, что я не тронул публичный метод `render`, в моей программе 
ничего не сломается. Вот почему `ООП` делает ваш код не только красивым, логичным, но и надёжным.

Но, конечно же, мы за эту стабильность и удобство "расплачиваемся" тем, что обязаны осознанно выполнять несколько 
требований:
...
Я вам ответственно заявляю (а мне верить можно), эти требования выполнять совсем не сложно. Ненапряжно, короче. 
Так что, друзья, выполняем и радуемся! ;) 
...
Скажу вам грустным голосом, что, конечно же, эти правила нарушают, — этим грешат и умудренные опытом ООП-адепты. Но 
лучше эти два запрета выполнять. Всегда.
...
В `Javascript` класс может наследоваться от одного или нескольких "родителей", но в отличие от реальной жизни, 
перенимает **сразу все** поля и методы своих "родителей". Технически это происходит так:
```javascript
...
var myTextarea = new Textarea({nameAttr: 'comment'});
myTextarea.render(document.body);
```

Мы только что объявили новый класс `Textarea`, который унаследовали от _одного_ класса `Input`.
...
_Конечно, этого же можно добиться на чистом `Javascript` с помощью прототипов. Но я не хочу этим заниматься. Если 
вам очень любопытно, почитайте статью Ильи Кантора 
["Наследование классов в JavaScript"](https://learn.javascript.ru/class-inheritance). Но лучше не надо. Пока не надо
 перегружать мозг ненужной информацией._

В нашем примере у класса `Textarea` нет собственных методов, кроме `__constructor`, который мы добавили, чтобы
вызвать точно такой же метод в базовом классе:
```javascript
var Textarea = inherit(Input, {
    __constructor: function (params) {
        this.__base(params);
...
Я думаю, вы уже поняли, что базовым классом называют тот, от которого наследуют. А производным — тот, который 
наследуется от базового. Запомним эти термины.

Переопределение базового метода весьма простая операция — создаем в прозводном классе метод с точно таким же 
именем, как в базовом. Производный метод будет словно перетирать базовый метод. 
...
Мы создали класс `Textarea`, который наследует от класса `Input` все его свойства и методы. А потом у класса 
`Textarea` описали метод `_createDOMElement`, который выглядит почти так же, как его тезка в классе `Input`. Отличие 
состоит в том, что `Textarea._createDOMElement` создаёт `<textarea>`-ноду, а не `<input>`.
