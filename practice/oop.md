...
`Классов` и `Объектов`. Непонятно? Вот и мне было непонятно, когда я знакомился с этой темой впервые :)

В классических учебниках эту тему пытаются объяснять на примере автомобиля.

Об автомобиле, — пишут в учебниках, — можно говорить, как о конкретной машине. А можно иначе — как о неком типе 
объектов нашего мира. Когда мы рассуждаем о автомобиле как о _типе_ объектов, то есть как о неком абстрактном понятии  
"автомобиль", с точки зрения `ООП` мы называли бы такой тип другим словом - `Класс Объектов`.
  
Дальше в учебниках предлагается выйти на улицу и оглядеться. Что мы увидим? Припаркованные машины, спешащих по 
тротуарам людей. В небе самолёт увидим, — белый след за собой оставляет. Обычная картина.
...
Каждый из этих ~~типов~~ классов имеет общие характерные признаки. К примеру, машина имеет 4 колеса, двигатель, руль, 
 ...
 характерные признаки, а, к примеру, лифты не имеют **всех этих** признаков (но имеют какие-то свои признаки), 
 следовательно они принадлежат к другому классу объектов.

А `Объект` — это, мол, не абстрактный автомобиль, в очень конкретная машина, созданная на конкретном автомобильном 
заводе. Машина имеет все характерные признаки класса автомобиль (4 колеса, двигатель, руль...), но это уже не 
...
лишь описывает свойства и умения, которыми будет обладать конкретный объект, когда его создадут по этим правилам. То
 есть, `Объект - это уже конкретная реализация этого класса`.
