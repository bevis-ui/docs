...
var Input = inherit({
    ...
    /**
     * Создаёт DOM-элемент <input type="text" name="">
     *
     * @private
...
`JSDoc`, в которм так же указал, что этот метод приватный.
...
конструкторне делает всё сразу сам - он лишь делегирует полномочия другим методам класса. Конструктор стал 
...
И если завтра на Тех.Обслуживании механик выкинет один ремень и вставитт новый, я об этом даже не узнаю — мне 
...
объекта", мы получаем возможность хоть трижды в день менять реализацию приватных методов каждого класса. Это никак не
 поломает код основной программы. Только не трогайте публичные методы!
