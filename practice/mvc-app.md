...
```
Я привёл не весь файл, а только фрагмент. Но в этом коротком фрагменте описана вся инструкция сборщику, где он 
должен искать исходники — только в папках  `blocks`, `core`, `pages`. 

Так как мы изменили файловую структуру проекта, следует поправить и этот фрагмент. Он должен выглядеть так:
```javascript
"enb": {
    "sources": [
        "client/blocks",
