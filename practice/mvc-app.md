...
Все новые изменения я пометил комментариями.
`client/pages/index-page/controllers/page-controller.js`
```javascript
modules.define(
    'page-controller',
    ...
    var PageController = inherit({
        __constructor: function () {
            ...
            this._authModel = new AuthModel();  // <---- Создали экземпляр Модели Авторизации
            this._authModel.on('user-data-saved', this.start, this); // <--- Слушаем событие на модели
        ...
        start: function () {
             ...
             * @type {Boolean}
             */
            var isAuthorized = this._authModel.isAuthorized(); // <--- Спрашиваем у Модели, авторизован ли пользователь
...
Нам осталось только создать модель и приложение готово. так давайте создадим :)
