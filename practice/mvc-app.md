...
5. Когда пользователь введет данные и пароль, передать их в бекенд и слушать ответ от бекенда.

6. Если бекенд сказал, что сумел сохранить данные, перейти к п.№3 и выполнить его без перезагрузки страницы. 
 
Вполне себе задача. Несложная. При этом, послужит хорошей иллюстрацией.

_Для упрощения иллюстрации мы пока не будем реально передавать данные в какой-то бекенд. Мы создадим Модель для этого,
 но вместо передачи данных в базу данных, она будет сохранять логин в куку. Если это произойдёт успешно (я правда не 
...
Теперь, когда задача ясна, набросаем алгоритм взаимодействия контроллера, вьюх и модели.

1. Когда контроллер стартовал, он спрашивает у Модели, авторизован ли пользователь.
2. Модель читает какую-то куку (позже будет спрашивать у бекенда).
3. Если кука есть:
  * Модель возвращает в Контроллер какой-то флаг "авторизован"
  * Контроллер ренедрит вью с контентным блоком.
4. Если куки нет:
  * Модель возвращает в Контроллер какой-то флаг "неавторизован"
  * Контроллер ренедрит вью с формой авторизации и подписывается на событие от формы типа "пользователь попытался 
авторизоваться с помощью формы"
  * Форма в свою очередь подписывается на похожее сообщение от Инпута, а когда получит его, пробросит его через себя.
  * А чтобы ИНпут сообщил о воде пользвательских данных, мы программируем ИНпут так, чтобы он слушал нажатие кнопок на
 клавиатуре пользователя, и в тот момент, когда пользователь нажмёт кнопку Enter при установленом фокусе, 
 Инпут выбросит своё событие что-то вроде "пользователь попытался авторизоваться с такими-то данными".
        
Должна получиться такая цепочка событий:
 
1. Инпут слушает нажатие кнопки на клавиатуре. 

2. Если нажали кнопку Enter, Инпут емитит событие "пользователь попытался авторизоваться" и методом emit передаёт 
данные, которые ввел пользователь.

3. Это событие получает Форма, которая с помощью метода emit выбрасывает уже своё событие (которое может называться так 
же, как пришло у Инпута) и передаёт в методе те данные, которые забрал у ИНпута.

4. Это событие от формы получает Контроллер страницы. Получает данные. 

5. Передаёт в Модель, чтобы та сохранила их в базе данных.

6. Модель как бы сохраняет данные в базе (ан самом деле в куке) и выбрасывает событие "Пользователь добавлен в базу".

7. Контроллер подписан на это событие. Когда контроллер услышал такое событие от Модели, он удаляет форму авторизации
 и рендерит основной контент страницы.
...
1. Когда контроллер стартовал, он должен узнать, авторизован ли пользователь.
...
Это называется `Single Page Application`. Именно этого мы и хотим.
...
modules.define(
    ...
    function (
        ...
        inherit,
        $,
        Sidebar     // <-- Приняли его в качестве аргумента
    ) {
    ...
    var PageController = inherit({
        ...
        start: function () {
            ...
            if (isAuthorized) {
                // 2. Если авторизован, контроллер ренедрит вью с контентным блоком.
                var sidebarView = new Sidebar({   // <-------------------------------- Создали блок на лету
                ...
                sidebar.getDomNode().appendTo($('body')); // <---------------------- Внедрили в DOM-дерево
            } else {
                // 3. Если не авторизован, контроллер ренедрит вью с формой авторизации.
            }
        }
...
Мы на лету (в памяти браузера) генерим блок `Sidebar`, а потом внедряем его внутрь `<body>` с помощью jQuery-метода 
`appendTo`.

Если сейчас обновить страницу, всё сломается, потому что у блока `sidebar` нет клиентского `js`-модуля. Нет же файла 
`client/views/sidebar/sidebar.js`, не существует. Мы его не создавали на предыдущих уроках, он не был нужен. А раз 
нет клиентског модуля, то конструкция `var sidebarView = new Sidebar()` бессмысленна. Нет файла 
`client/views/sidebar/sidebar.js` — значит нет модуля `sidebar`, значит нет класса `Sidebar`, следовательно, не от 
чего делать инстанс.  
 
Создадим модуль. Выполните команду в терминале:
```
echo "modules.define(
    'sidebar',
    ['inherit', 'block'],
...
```
Теперь контрукция `var sidebarView = new Sidebar()` обретает смысл. Как вы помните, в тот момент, когда мы сказали 
new Sidebar() и передали внутрь опции, запускается конструктор модуля в файле `client/views/sidebar/sidebar.js`, 
который запускает генерацию `HTML`-тегов с помощью шаблонов, описанных в файле `client/views/sidebar/sidebar.bt.js`
 
Да что я объясняю, вы это это знаете. 

_Знаете ведь? Если не очень, а перечитайте [раздел про генерацию блока на клиенте](https://github.com/bevis-ui/docs/blob/master/practice/yblock.md#Способ-2-Динамический-с-использованием-btjson)_

Теперь обновив страницу в браузере, мы увидим список ссылок про Бивис. Отлично, ветка `if` в контроллере сработала. И
 этот user case мы реализовали.
 
 Теперь нам нужно представить, что пользователь не авторизован. Для этого переменную `isAuthorized` мы установим в 
 `false`. Теперь в операторе `if-else` выполнится ветка `else`. напишем код для неё. Что нужно сделать? Отредериь 
 форму. рендерим:
`client/pages/index-page/controllers/page-controller.js`
```javascript
modules.define(
    ...
    function (
        ...
        $,
        Sidebar,
        Form,       // <-- Приняли форму в качестве аргумента
    ...
    var PageController = inherit({
        ...
        start: function () {
            ...
            if (isAuthorized) {
                ...
                sidebar.getDomNode().appendTo($('body')) ;
            } else {
                // 3. Если не авторизован, контроллер ренедрит вью с формой авторизации.
                var formAuth = new Form({                 // <-------------------------------- Создали блок на лету
                    titleText: i18n('form', 'title-text') // <-------------------------------- Передали в него btjson
                });
                formAuth.getDomNode().appendTo($('body')) ;
            }
        }
    });
