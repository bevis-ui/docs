...
Теперь самая простая часть в Модели - это создать те публичные методы, которые мы хотим вызывать из `PageController`.
 Если мне не изменяет память, мы хотели дергать только два метода: `isAuthorized` - чтобы получить из него булевское
  значение и `save`, чтобы сохранить данные в бекенде.
...
Теперь метод `save` как бы сохраняет данные (на самом деле меняет флаг), после чего эмитирует событие `saved`.
...
Кажется, что теперь наша страница готова к полноценному циклу работы. Когда мы обновим страницу, сначала пользователь
 не авторизован, контроллер покажет форму. Мы нажмём в инпуте формы кнопку `Enter` - данные из формы попадут в модель в
  метод `save`, после чего модель выкинет сообщение `saved`. 
...
Сохранение авторизационных данных происходит в методе `save`. Сейчас он принимает аргумент типа `Object`, показывает 
...
Мне кажется, получится хорошо, если метод `save` выступит в роли ручки, которую можно дёрнуть "снаружи", но сама 
...
Вся грязная работа по сохранению будет происходить в приватном методе `_setUserData`, а метод `save` будет работать 
...
страницы форма исчезает, а контентный блок появляется. А в заголовке нпаисано: "Привет, Вадим!"
   
Обращаю ваше внимание на то, что в Модели две группы методов - публичные и приватные. Публичные методы `get` и `set` 
работают через своих приватных слуг - методы `_getUserData` и `_setUserData`. На следующем занятии мы будем сильно 
править именно эти приватные методы. Мы будем реализовать настоящее общение с бекендом.
  
Под бекендом я подразумеваю Базу Данных + специальные скрипты-провайдеры данных, которые умеют обращаться к базе, 
чтобы что-то передать в базу или что-либо прочитать из базы. В Яндексе мы такие провайдеры привыкли называть "Ручки".
 Когда я слыш это слово на работе, я представлю себе ручки кикера - настольного футбола. Мы их крутим-крутим, так и 
 эдак, иногда с остервенением, совсем, как умелые игроки, — например, Вадим [Pepelsbey](pepelsbey.com) Макеев ;)
