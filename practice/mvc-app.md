...
Пришло время слушать события от формы авторизации. Напомню, как выше выглядит этот фрагмент в алгоритме:
  * [done] Контроллер ренедрит вью с формой авторизации
  * Контроллер подписывается на событие от формы типа "пользователь попытался авторизоваться с помощью формы"
  * [done] Форма в свою очередь подписывается на похожее сообщение от Инпута, а когда получит его, пробросит его 
  через себя.
  * [done] А чтобы ИНпут сообщил о воде пользвательских данных, мы программируем ИНпут так, чтобы он слушал нажатие 
  кнопок на клавиатуре пользователя, и в тот момент, когда пользователь нажмёт кнопку Enter при установленом фокусе,
   Инпут выбросит своё событие что-то вроде "пользователь попытался авторизоваться с такими-то данными".
   
Из четырёх пуктов не сделан только второй. Остальное готово :) Первый пункт сделали только что. Третий и 
четвертый мы сделали ещё тогда, когда учились писать клиентский `js` для блока: [Слушаем события на 
...
Сделаем третий пункт — подпишемь контроллер на событие от формы типа "пользователь попытался авторизоваться с 
помощью формы". То есть форма должна такое событие выбрасывать, а контроллер ловить. Делаем точно так же, как [форма 
слушала событие от Input](https://github.com/bevis-ui/docs/blob/master/practice/yblock.md#Подписка-на-произвольное-событие)
...
Теперь контроллер слушает события от формы и получает от неё данные. Это значит, что когда пользователь введёт логин с  
паролем в форме авторизации и, стоя в каком-то из полей, нажмёт клавишу `Enter`, информация из текстовых полей по 
...
данные  *только с паролем*. А если пользователь нажал кнопку `Enter` в поле для логина - в контроллер придут  
данные  *только с логином*. 
  
А как надо? Опишем алгоритм для этого фрагмента работы формы:
...
пользоваталем в текстовые поля, прямо из инпутов с помощью их публичных методов `getValue`._
...
И теперь перейдём к созданию Модели.
