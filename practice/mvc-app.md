...
Для упрощения иллюстрации мы пока не будем реально передавать данные в какой-то бекенд. Мы создадим Модель для этого,
 но вместо передачи данных в базу данных, она будет сохранять логин в куку. Если это произойдёт успешно (я правда не 
 знаю, что может пойти не так, при сохранении куки ;), вернёт ответ в контроллер страницы. 
 
Не будет эта Модель и читать данные из бекенда. Вместо похода в бекенд за авторизационными данными Модель будет 
ходить в куку и читать логин там. Если логин в куке есть, Модель вернёт в контроллер этот логин, в противном случае 
вернет сообщение, что "Пользователь не авторизован".

Сначала реализуем так, чтобы не отвлекаться на тему ["Как взаимодействовать с бекендом"](backend-requests.md). Позже 
...
переделать потом. В конце концов, не в этом ли вся сила `MVC`, не ради этого ли разделение ролей и придумано?

Теперь, когда задача ясна, напишем алгоритм работы Контроллера, вьюх и модели.
