...
5. Контроллер передаёт полученные данные в Модель, чтобы та сохранила их в базе данных.
...
Это означает, что когда молель успешно сохранит данные, она должна эмитировать событие `user-data-saved`. Хорошо, 
...
Всё шло по плану, но после сабмита формы она не исчезла, а под ней отобрался сайдбар. ну это понятно почему - когда 
второй раз вызвался метод `start`, он не очистил DOM-дерево, а просто добавил сайдбар.

Ну, несложно исправить...
...
Всё, мы сделали самое главное. Осталась малость - по настроящему читать авторизационные данные и сохранять их. 
