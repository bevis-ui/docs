...
_Контроллеру вообще не интересно знать, где Модель берет данные и куда сохраняет. Поэтому публичные методы Модели, к
 которым обращается Контроллер, напишем так, чтобы их больше не приходилось менять. Сделаем так и будет нам хорошо. 
...
При этом одну минуту назад мы поставили себе задачу, что публичный метод `save` нужно продумать, написать и больше не 
...
методе, приватном, снаружи недоступном.
