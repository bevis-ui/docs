...
4. Если куки нет:
  ...
  * А чтобы ИНпут сообщил о воде пользовательских данных, мы программируем ИНпут так, чтобы он слушал нажатие кнопок на
...
Пришло время слушать события от формы авторизации. Напомню, как выше выглядит этот фрагмент в алгоритме:
  * Контроллер ренедрит вью с формой авторизации
  * Контроллер подписывается на событие от формы типа "пользователь попытался авторизоваться с помощью формы"
  * Форма в свою очередь подписывается на похожее сообщение от Инпута, а когда получит его, пробросит его 
  через себя.
  * А чтобы Инпут сообщил о воде пользовательских данных, мы программируем ИНпут так, чтобы он слушал нажатие 
  кнопок на клавиатуре пользователя, и в тот момент, когда пользователь нажмёт кнопку Enter при установленом фокусе,
   Инпут выбросит своё событие что-то вроде "пользователь попытался авторизоваться с такими-то данными".
   
Из четырёх пуктов не сделан только второй. Остальное готово — первый пункт сделали только что, а третий и 
четвертый мы сделали давным-давно, ещё когда учились писать клиентский `js` для блока: [Слушаем события на 
Input](https://github.com/bevis-ui/docs/blob/master/practice/yblock.md#Навешивание-и-обработка-событий), [Form 
слушает события от Input](https://github.com/bevis-ui/docs/blob/master/practice/yblock.md#Подписка-на-произвольное-событие).

Сделаем третий пункт — подпишем контроллер на событие от формы типа "пользователь попытался авторизоваться с 
...
1. Когда контроллер стартовал, он спрашивает у Модели, авторизован ли пользователь. Сохраняет эту информацию в 
переменной `isAuthorized` 

2. Если пользователь не авторизован:
  * Контроллер подписывается на событие от формы типа "пользователь попытался авторизоваться с помощью формы"
  * Из обработчике мы передаём данные полей в Модель.
  * Контроллер подписывается на событие от Модели "данные сохранились успешно"
  * Когда контроллер услышит событие от модели, он вызовет свой метод `start`, таким образом приложение начнёт 
  работать как бы сначала. То есть проверит, авторизован ли пользователь. Он будет уже авторизован, следовательно 
...
сложно, ответим на простой вопрос: "Модель нужна только на _этой_ странице, или она будет использоваться на всех 
страницах?"

Так как мы будем создавать модель для получения/сохранения авторизационных данных, то скорее всего модель может быть  
использована на любой странице. Значит модель создадим в директории `client/models/`. 

#### Файл для модели, зависимости и сборка
Такой директории ещё нет, создадим:
...
_А если вдруг запятовали, можно освежить знания в разделе 
[Модули](https://github.com/bevis-ui/docs/blob/master/practice/yblock.md#Модули) или на странице автора: 
...
`data`-атрибутов в тегах и всякое другое, что нужно для работы с `DOM`-объектами.
