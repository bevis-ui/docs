...
_**Сразу сделаем правильно!**_ 
_Контроллеру вообще не интересно знать, где Модель берет данные и куда сохраняет. Поэтому публичные методы Модели, к
 которым обращается Контроллер, напишем так, что их больше не приходилось менять. Сделаем так и будет нам хорошо. 
 Потому что завтра, когда мы перепишем Модель, чтобы она сохраняла или получала данные из 
 бекенда, а не из куки, нам уже не придётся переписывать Контроллер._
...
С какой начинать? А, всё равно! Начнём с первой.
...
При этом мы поставили себе одну минуту назад, что публичный метод `save` нужно продумать, написать и больше не 
изменять. Значит, мыслю я вслух, нельзя писать так, чтобы непосредственное сохранение происходило в этом методе. 
Метод `save` должен выступать в роли ручки, которую можно дёрнуть "снаружи", а вся работа будет происходить в другом 
методе, приватном, недоступном снаружи.
...
Работает. Я вижу форму авторизации, ввожу что-то в поля и нажимаю `Enter`. Визуально ничего не происходит 
(неудивительно, мы же ничего не написали для того, чтобы визуально что-то менялось), но кука `authorization` 
создаётся. Я проверяю с помощью браузерного расширения [Edit This Cookie](http://www.editthiscookie.com). Оно 
работает под `Chrome`, новую `Opera`, `Yandex.Browser` и все другие браузеры, основанные на движке `Blink`.

Всё, мы закончили сохранять данные. Переходим ко второй задаче Модели.
