...
`isAuthorized` - чтобы получить из него булевское значение и `save`, чтобы сохранить данные в бекенде (или в 
куке, неважно пока).
    ...
    function (
        ...
        var AuthModel = inherit(YEventEmitter, {
            ...
            save: function (data) {
                ...
                // Должны сохранить данные в куку, пока эмулируем с помощью флага
...
_Этим мы займёммся на следующем практическом занятии, посвященном [взаимодействии с бекендом](backend-requests.md)._  

На этом же занятии вместо передачи данных в бекенд, пусть наша Модель сохраняет логин и пароль в куку. Это небезопасно, 
но нам для практики подойдёт. Если сохранение куки произойдёт успешно (я правда не знаю, что может пойти не так при 
сохранении куки ;), Модель выкинет сообщение об успехе, а Контроллер его услышит. 
 
Не будет эта Модель и читать данные из бекенда. Вместо похода в бекенд за авторизационными данными Модель будет 
ходить в куку и читать логин там: если логин в куке есть, Модель вернёт в Контроллер этот логин, в 
противном случае вернет какое-то сообщение, например, "Пользователь не авторизован".

_**Сразу сделаем правильно!**_ 
_Контроллеру вообще не интересно знать, где Модель берет данные и куда сохраняет. Поэтому сразу продумаем публичные 
методы Модели, к которым будет обращаться Контроллер для получения или сохранения данных. Продумаем, создадим. И больше
 их менять не будем. Никогда. Тогда завтра, когда мы перепишем Модель, чтобы она сохраняла или получала данные из 
 бекенда, а не из куки, нам уже не придётся переписывать Контроллер._

Приступим. У Модели две задачи. С какой начнём?
* Модель сохраняет данные
* Модель читает данные

Ну, так и начнём, по порядку.

### Модель сохраняет данные

В ветке `mvc-application` в папке `client/core/cookie` есть модуль для работы с куками. Мы взяли плагин 
Клауса Хартла (Klaus Hartl, http://stilbuero.de), и завернули в  модуль. Спасибо вам, Клаус, отличный плагин!  

Для сохранения данных Модель предоставляет публичный метод `save`, он принимает один параметр типа `Object`. 
Здесь и сохраним.
