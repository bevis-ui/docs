...
_Кстати, а вы знакомы с правилами документирования `javascript`-кода? Исчерпывающие сведения о `JSDoc` вы найдёте на 
[http://usejsdoc.org](http://usejsdoc.org)_
...
После этого я вызвал приватный метод `_render`. Остановимся на нём подробнее. 

Из названия и `JSDoc` следует, что метод занимается рендерингом всех контролов в форме. Правда, у нас пока только один 
контрол, но позже станет больше. Рендерингом мы привыкли называть отображение какого-то блока в `DOM`-дереве. То 
есть,  разумеется, метод `_render` ничего не рендерит в прямом значении этого слова, он лишь вставляет какие-то блоки в 
`DOM`-дерево. А рендерит их (отображает, навешивает стили и т.д) уже сам браузер. Слава богу, нам этого делать не надо!

Чтобы вставить блок в `DOM`-дерево мы пользуемся `jQuery`-методами `append` и `appendTo`. Они работают одинаково с 
единственной разницей:
```javascript
'что-вставляем'.appendTo('куда-вставляем');
'куда-вставляем'.append('что-вставляем');
```

На месте 'что-вставляем' и 'куда-вставляем' должны быть `jQuery`-представления реальных `DOM`-узлов. Теперь тот же 
метод с правильными, но избыточными комментариями:  

```javascript
/**
 * Вставляет jQuery-представления всех контролов в jQuery-представление формы
 */
_render: function () {
    // Получить jQuery-представление формы 
    var form = this.getDomNode();
    
    // Получить jQuery-представление инпута 
    var greeting = this._greetingInput.getDomNode();

    // Инпут добавить внутрь формы
    greeting.appendTo(form);
}      
```
 
Не знаю, не смотрел внутрь `jQuery`- метода `appendTo`, как именно он добавляет одну `DOM`-ноду в другую. Факт в том,
 что после `appendTo` инпут окажется в дереве документа, и мы сразу увидим его на странице. И на нём уже будут 
 навешаны все слушатели событий, то есть инпут будет готов к работе сразу же. То есть как только я "заапендил" 
 экземпляр  инпута в `DOM` дерево, он тут же проинициализовался и готов к работе.
