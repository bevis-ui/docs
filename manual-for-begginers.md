...
В этом документе мы расскажем вам всё. Растолкуем, из чего состоит `BEViS`. Познакомим с нашими замечательными
инструментами и научим вас ими пользоваться. И мы обещаем, вы ухватите суть так же легко,
как ребёнок запоминает  лицо матери, то есть без каких бы то ни было  особенных умственных усилий. Потому что
`BEViS` - это очень просто :)
...
У формы одна задача - собрать в себя данные и отправить на сервер. Поэтому давйте считать,
...
вы не знакомы с "метологиями" web-разработки, вы конечно же используете в своей верстке блоки, только вполне возможно,
...
вы в  совершенстве овладете Бивисом, потому что Бивис сильно похож на олдскульный способ верстки; потому что все
...
Схма взаимодействия браузера и сервера примерно такая:
...
элемемнтов строить самого себя.
...
Нам этого хватало. В действительсноти, такой декларации может хватать половине блоков на реальной боевой странице
...
о чём вы говорите? Когда вы хотите чуть-чуть переопределить стуктуру блока, чтобы сделать "такой же,
...
А сейчас нам нужно вернуться к стилевому офоормлению блока.
...
```html
    <div class="y-button _disabled"></div>

    <div class="y-button _focused"></div>
```
...
Как мы приудмываем имена состояниям?
...
```
    <div class="y-popup _orientation_top"></div>
```
...
`CSS` и `Javascript`. А сама схема взаимодействия сервера и клиента не изменилась и знакому каждому,
...
---
В тексте мы ссылались на эти ресурсы, вот они.

* [BT документация](https://github.com/enb-make/bt)
* [YBlock документация](https://github.yandex-team.ru/islets/docs/blob/master/YBlock.md)
* [Что мы думаем о копипасте?](https://github.yandex-team.ru/islets/docs/blob/master/BEM-vs-BEViS.md#copypaste)
